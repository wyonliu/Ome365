...
> **注**：Phase 2c 的 oidc / wecom provider 实现还未合入（卡在用户拍板 5 问）。当前企业部署可先跑 `basic` 过度，等 SSO provider 合入后直接改 `auth.provider` 即可。
...
```bash
mkdir -p $OME365_HOME/tenants/acme/users/wyon
...
cat > $OME365_HOME/tenants/acme/users/wyon/profile.json <<EOF
{
  "uid": "wyon",
  "email": "user@example.com",
  "display_name": "用户",
  "vault_path": "/data/vaults/acme/wyon",
  "roles": ["admin"]
}
EOF
```
