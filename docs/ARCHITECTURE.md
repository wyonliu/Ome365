...
│  ├─ $VAULT/Projects/Acme/reports/**（acme诊断报告）       │
...
**Tier 1 · 工作上下文特化**（acme驾舱）
- Interviews 抽屉（TicNote pipeline，36 份访谈，87.4 万字）
- Reports 驾舱（6 大板块，内部诊断 41 份，处方卡/Flagship/三大支柱/metrics）
...
- [ ] `ome365-acme-plugin`：驾舱、TicNote、Reports
...
**局限**：
- 驾舱/Interviews/Reports 是acme特化，同事看到可能空
- Life/Growth 要从 0 养
- 所有数据在本地，不要写公司机密
...
**绝不推送**（已 .gitignore）：
- `Acme/`, `TicNote/`, `Hiring/`, `Life/`, `Insights/`, `Projects/`（公司 + 私人数据）
