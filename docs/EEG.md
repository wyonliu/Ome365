...
通用 LLM 不知道：
- 「黄阳」在龙湖语境里是 ASR 对「—」的误读（不是另一个 CTO）
- 「C1」在龙湖是住宅航道，不是「Customer 1」
- 「Lilalo」=「利拉洛」=「利拉漏」=「利拉落」指同一个 C 端智能体产品
- 「群总 / — / 涛哥」分别是谁

**企业智能体要靠谱，第一块必须硬的就是实体层**。否则：
- RAG 检索会漏（搜「—」召回不到「黄阳」的录音段）
- 生成会错（把「黄阳」写成新人）
- 展示会乱（同一个人出现两个头像卡片）
...
| `organization` | 千丁、C1 供应链、智慧空间 BU | name / aliases / parent / type（航道/BU/部门） |
| `product` | Lilalo、Dify、Claude Code、竞优 | name / aliases / vendor / category |
| `term` | A4S、珑珠、珑豆、LongScale、Curve 1-7 | name / aliases / definition / scope |
...
POST /api/entities/resolve                # 别名→规范名：{ text: "黄阳总提了一下" } → { canonical: "—总提了一下", matches: [...] }
...
| **Insights 合成** | 生成摘要前 canonicalize，避免「黄阳和—是两个人」 |
...
**血泪教训**：2026-04-17 前，「黄阳」被当真人三次。所以：
...
- ✅ `seed_entities.py`: 从 `Contacts/people/` + `reference_longfor_terminology.md` 一次性播种
