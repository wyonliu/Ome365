...
async def eeg_asr(tenant: str = "acme"):
...
async def eeg_resolve_api(body: dict):
    ...
    tenant = body.get("tenant") or "acme"
...
async def get_interviews():
    ...
    for d in sorted(TICNOTE_DIR.iterdir(), reverse=True):
        ...
        for fp in sorted(d.glob("*.md")):
            ...
            # New format: "acmeBU-智慧建造-—·主题·2026-04-08"
            # Legacy:     "acme数科-—-主题-04月08日"
            # Split on · first to get prefix, then split prefix on -
            seg0 = stem.split("·")[0]  # "acmeBU-智慧建造-—" or "C1供应链-—"
            ...
            cat = "未分类"
            low = seg0.lower()
            if low.startswith("集团") or low.startswith("acmeceo") or low.startswith("acmehrd"):
                cat = "管理层"
            elif _re.match(r'^C\d', seg0) or _re.match(r'^N\d', seg0) or _re.match(r'^acmeC\d', seg0) or _re.match(r'^acmeN\d', seg0):
                cat = "航道"
            elif "终面" in seg0 or "面试" in seg0:
                cat = "面试"
            elif "acmeBU" in seg0 or "acme战略" in seg0 or (seg0.startswith("acme") and any(k in seg0 for k in ["物管", "财务", "签零", "员工", "数科", "建造", "建管", "空间", "资管", "城服", "IDC", "营销", "运营", "AI创新"])):
                cat = "BU"
...
# ── API: Reports ─────────────────────────────────────
REPORTS_DIR = VAULT / "Projects" / "Acme" / "reports"
...
async def insights_synthesize(body: dict):
    ...
    user = f"""{context}
{focus_hint}
请基于以上语料，为用户（北大计算机硕士、AI 产品与技术领导者、acme CTO）输出以下 JSON：
...
{{
  ...
  "diagnosis": {{
    ...
    "blind_spots": ["2-3 条用户自己可能没注意到的事情"]
  }},
  "opportunities": [
    {{"title": "机会名", "why_now": "为什么此刻成立（结合语料证据）", "action": "一句话行动建议"}}
  ],
  "questions": [
    "3-5 个值得用户下周静下来思考的开放式问题"
  ]
}}
...
# ── ACME COCKPIT · acme战略驾舱 ───────────────────
# Data source: a single markdown file (Vault/Acme/cockpit.md).
...
ACME_DIR = VAULT / "Acme"
COCKPIT_FILE = ACME_DIR / "cockpit.md"
...
@app.get("/api/acme/cockpit")
async def acme_cockpit():
...
@app.get("/api/acme/cockpit/raw")
async def acme_cockpit_raw():
    if not COCKPIT_FILE.exists():
...
@app.post("/api/acme/cockpit/save")
async def acme_cockpit_save(body: CockpitSaveBody):
    # Keep a lightweight backup (previous version) before writing
    if COCKPIT_FILE.exists():
        backup = ACME_DIR / ".cockpit.backup.md"
...
@app.get("/api/acme/cockpit/export")
async def acme_cockpit_export():
...
.triad-stack{display:flex;flex-direction:column;gap:22px}
...
<div class="page">
  ...
  {''.join(blocks)}
  <footer class="footer">
    Acme Qianding Strategy Cockpit &nbsp;·&nbsp; {updated}<br>
...
<div class="ch-row"><b>acme位</b>{esc(c.get('acme_role',''))}</div>
</div>""")
...
<div class="tri-foot">负责方 · {esc(t.get('owner',''))}</div>
        ...
        return f'<div class="triad-stack">{"".join(cards)}</div>'
...
# Deploy: set SHARE_BASE_URL=https://ome365.example.com
import hashlib
...
if __name__ == "__main__":
    ...
    for d in ["Journal/Daily","Journal/Weekly","Journal/Monthly","Journal/Quarterly","Notes","Decisions","Contacts/people","Projects","AI-Logs","Templates","Memory","Memory/insights","Insights","Life","Acme"]:
